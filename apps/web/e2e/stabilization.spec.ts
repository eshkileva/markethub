...
test.describe('stabilization API smoke', () => {
  test('geo cities cap without q and finds Omsk with q', async ({ request }) => {
    const allRes = await request.get(`${E2E_API_BASE}/v1/geo/cities?country=RU`);
    ...
    const searchRes = await request.get(
      `${E2E_API_BASE}/v1/geo/cities?country=RU&q=${encodeURIComponent('Омск')}`,
    );
  ...
  test('cars and moto catalogs are leaf-scoped', async ({ request }) => {
    const carsRes = await request.get(`${E2E_API_BASE}/v1/catalogs/cars/brands`);
    await assertOk(carsRes, 'cars brands');
    const motoRes = await request.get(`${E2E_API_BASE}/v1/catalogs/moto/brands`);
  ...
  test('phones root filter includes smartphones demo listing', async ({ request }) => {
    const catsRes = await request.get(`${E2E_API_BASE}/v1/categories`);
    ...
    const phonesFeed = await request.get(
      `${E2E_API_BASE}/v1/listings?categoryId=${phones.id}&page=1&pageSize=20`,
    );
    ...
    const motoFeed = await request.get(
      `${E2E_API_BASE}/v1/listings?categoryId=${moto.id}&page=1&pageSize=20`,
    );
  ...
  test('creating listing on root category returns 422', async ({ request }) => {
    ...
    const { accessToken } = await registerVerifiedUser(request, {
      email,
      password: 'password12',
    ...
    const createRes = await request.post(`${E2E_API_BASE}/v1/listings`, {
