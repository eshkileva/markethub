...
export const listings = pgTable(
  'listings',
  {
    ...
    listingTrustScore: integer('listing_trust_score'),
    aiRiskLevel: text('ai_risk_level'),
    aiAssessment: jsonb('ai_assessment').$type<Record<string, unknown> | null>(),
    aiAssessedAt: timestamp('ai_assessed_at', { withTimezone: true }),
  ...
  (table) => [
    ...
    index('listings_pending_moderation_idx').on(table.aiRiskLevel, table.createdAt),
    check(
      'listings_status_chk',
